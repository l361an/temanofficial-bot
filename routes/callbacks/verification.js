...
import { listActiveVerificators, getAdminByTelegramId, listAdmins } from "../../repositories/adminsRepo.js";
...
import {
  buildPartnerApprovedMessage,
  buildOwnerApprovedMessage,
  buildOfficerApprovedMessage,
...
async function notifyApprovedToOfficers(env, {
  ownerId,
  nickname,
  username,
  telegramId,
  ...
  const recipients = (rows || []).filter((row) => {
    const role = String(row?.normRole || row?.role || "").trim().toLowerCase();
    return role === "owner" || role === "superadmin" || role === "admin";
  });

  const ownerText = buildOwnerApprovedMessage({
    nickname,
    username,
    telegramId,
    verificator: verificatorLabel,
  });

  const officerText = buildOfficerApprovedMessage({
    nickname,
    username,
    telegramId,
    verificator: verificatorLabel,
  ...
  for (const row of recipients) {
    const targetId = String(row?.telegram_id || "").trim();
    if (!targetId || sent.has(targetId)) continue;

    const role = String(row?.normRole || row?.role || "").trim().toLowerCase();
    const text = targetId === String(ownerId || "").trim() || role === "owner"
      ? ownerText
      : officerText;

    await sendMessage(env, targetId, text).catch((err) => {
      console.error("APPROVED OFFICER NOTIFY ERROR:", targetId, err);
    });
  ...
  PREFIX.push({
    ...
    run: async (ctx) => {
      ...
      if (action === "approve") {
        ...
        const aturanLink = await getSetting(env, "link_aturan").catch(() => "");

        await sendMessage(
          env,
          telegramId,
          buildPartnerApprovedMessage({
            linkAturan: aturanLink,
          }),
          { reply_markup: buildTeManMenuKeyboard() }
        ).catch(() => {});

        await notifyApprovedToOfficers(env, {
          ownerId: adminId,
          nickname: profile.nickname,
          username: profile.username,
          telegramId,
          verificatorLabel: vLabel,
        });
        ...
        await sendMessage(
          ...
          [
            ...
            approvedRes?.status_changed_at ? `Approved at: ${approvedRes.status_changed_at}` : null,
          ].filter(Boolean).join("\n"),
          {
